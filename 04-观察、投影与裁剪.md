...
> OpenGL 右手系看向 -Z；Direct3D/Vulkan 左手系看向 +Z。本笔记以 OpenGL 约定为主。
...
2. **完全不可见 (Trivial Reject)**：两个端点编码**按位与 (AND) ≠ 0** → 同在窗口某侧之外，丢弃
...
> ⚠️ 常见计算题：给定窗口和线段端点，写出算法每一步的编码和交点。
...
世界空间 (World)     — 场景中的全局位置
      ↓ View Matrix
观察空间 (View)      — 以相机为原点，看向 -Z
      ↓ Projection Matrix
裁剪空间 (Clip)      — 齐次坐标 (x,y,z,w)，用于裁剪
      ↓ Perspective Division (硬件自动)
NDC (归一化设备坐标)  — x,y,z ∈ [-1,1] 或 [0,1]
      ↓ Viewport Transform
屏幕空间 (Screen)    — 像素坐标
```
...
3. **Cohen-Sutherland 的两种快速判定** → Trivial Accept（code1=code2=0000）和 Trivial Reject（code1 & code2 ≠ 0）
...
7. **NDC 坐标范围在 OpenGL vs Vulkan/D3D 中的区别** → OpenGL z∈[-1,1], Vulkan z∈[0,1]; 但 x,y 都是[-1,1]
